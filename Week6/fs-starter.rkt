...
(define (sum-data--loe loe)
  (cond [(empty? loe) 0] ;atomic distinct empty
        [else
...
(check-expect (find--elem "F3" F1) false)
(check-expect (find--elem "F3" F3) 3)
(check-expect (find--elem "D4" D4) 0)
(check-expect (find--elem "D6" D6) 0)
(check-expect (find--loe "F2" (cons F1 (cons F2 empty))) 2)
(check-expect (find--loe "F3" (cons F1 (cons F2 empty))) false)
(check-expect (find--elem "F3" D4) false)
(check-expect (find--elem "F1" D4) 1)
(check-expect (find--elem "F2" D4) 2)
(check-expect (find--elem "F3" D6) 3)
(check-expect (find--elem "F1" D6) 1)
...
(define (find--loe n loe)
  (cond [(empty? loe) false]
        [else
         (if (not (false? (find--elem n (first loe)))) ;; is it found in first loe?
             (find--elem n (first loe))
